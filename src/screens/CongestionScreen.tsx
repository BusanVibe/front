...
const DEFAULT_CENTER = {
  latitude: 35.15612306382287,
  longitude: 129.15400865768683,
};
...
const CongestionScreen = () => {
  ...
  const [shouldShowCurrentLocation, setShouldShowCurrentLocation] =
    useState(false); // 현재위치 표시 여부
  const [isInitialLoad, setIsInitialLoad] = useState(true); // 초기 로드 상태
  // isLocationLoading은 LocationContext에서 관리
  const [realtimeStandardHour, setRealtimeStandardHour] = useState<
    number | null
  >(null);
  const [realtimeLevel, setRealtimeLevel] = useState<number | null>(null);
  const [realtimeByPercent, setRealtimeByPercent] = useState<number[] | null>(
    null,
  );
  const [visitorDistribution, setVisitorDistribution] = useState<
    {age: string; male: number; female: number}[] | null
  >(null);
  // 새로운 혼잡도 데이터 (주간/시간별)
  ...
  const pendingMoveToLocationRef = useRef<{
    lat: number;
    lng: number;
    show: boolean;
  } | null>(null); // WebView 로드 후 이동 예약
  const pendingCurrentLocationPingRef = useRef<{
    lat: number;
    lng: number;
  } | null>(null); // WebView 로드 후 현재위치 핑 예약
  ...
  const fetchRealtimeCongestion = async (placeId: number) => {
    try {
      ...
      const url = `https://api.busanvibe.site/api/congestion/place/${placeId}/real-time?standard-time=${encodeURIComponent(
        iso,
      )}`;
      ...
      if (res.ok && ok && data.result) {
        const r = data.result;
        const unwrapArrayList = (v: any) =>
          Array.isArray(v) && v.length === 2 && v[0] === 'java.util.ArrayList'
            ? v[1]
            : Array.isArray(v)
            ? v
            : [];
        setRealtimeStandardHour(
          typeof r.standard_time === 'number'
            ? r.standard_time
            : Number(r.standard_time || 0),
        );
        const levelRaw =
          r.realtime_congestion_level !== undefined
            ? r.realtime_congestion_level
            : r.real_time_congestion_level;
        setRealtimeLevel(
          typeof levelRaw === 'number' ? levelRaw : Number(levelRaw || 0),
        );
    ...
    try {
      ...
      if (res.ok && ok && data.result) {
        ...
        const mapped = [
          {
            age: '10·20대',
            male: toNum(r.male1020),
            female: toNum(r.female1020),
          },
          {
            age: '30·40대',
            male: toNum(r.male3040),
            female: toNum(r.female3040),
          },
          {
            age: '50·60대',
            male: toNum(r.male5060),
            female: toNum(r.female5060),
          },
  ...
  const fetchWeekCongestion = async (placeId: number) => {
    try {
      ...
      const iso = new Date(now.getTime() - now.getMilliseconds())
        .toISOString()
        .slice(0, 19); // yyyy-MM-ddTHH:mm:ss
      const url = `https://api.busanvibe.site/api/congestion/place/${placeId}/congestions?standard-time=${encodeURIComponent(
        iso,
      )}`;
      const res = await fetch(url, {
        method: 'GET',
      ...
      if (res.ok && ok && data.result) {
        ...
        const congestionsByDay = unwrapArrayList(r.congestions_by_day).map(
          (n: any) => Number(n) || 0,
        );
  ...
  const openDetailInAttraction = () => {
    try {
      const placeParam: any = {
        ...
        congestion_level: Number(
          (selectedLocation as any)?.congestionLevelNum || 0,
        ),
        is_like: false,
        type: 'SIGHT',
  ...
  const fetchPlaceDetail = async (placeId: number) => {
    try {
      ...
      if (res.ok && isSuccess && data.result) {
        const unwrapBigDecimal = (v: any) =>
          Array.isArray(v) && v[0] === 'java.math.BigDecimal' ? v[1] : v;
        const unwrapArrayList = (v: any) =>
          Array.isArray(v) && v.length === 2 && v[0] === 'java.util.ArrayList'
            ? v[1]
            : Array.isArray(v)
        ...
        const mapped = {
          ...
          reviewCount:
            typeof r.review_amount === 'number'
              ? r.review_amount
              : Number(r.review_amount || 0),
          distance: '',
          address: r.address,
        ...
        if (!isNaN(latitude) && !isNaN(longitude)) {
          if (webViewRef.current) {
            ...
            pendingMoveToLocationRef.current = {
              lat: latitude,
              lng: longitude,
              show: false,
            };
      ...
      if (response.ok) {
        ...
        if (data.is_success) {
          ...
          if (
            ...
            placeList.length === 2 &&
            placeList[0] === 'java.util.ArrayList'
          ) {
          ...
          if (Array.isArray(placeList)) {
            ...
            if (VERBOSE_CONGESTION_LOG) {
              ...
              console.log(
                `요청한 카테고리: ${category} → API 타입: ${getCategoryType(
                  category,
                )}`,
              );
            }
            ...
            if (VERBOSE_CONGESTION_LOG) {
              Object.keys(typeGroups).forEach(type => {
                ...
                typeGroups[type].forEach((place, index) => {
                  console.log(
                    `  ${index + 1}. ${place.name} - 혼잡도: ${
                      place.congestion_level
    ...
    try {
      ...
      if (location) {
        ...
        if (webViewRef.current) {
          ...
          pendingCurrentLocationPingRef.current = {
        ...
        Alert.alert(
          '위치 오류',
          '현재 위치를 가져올 수 없습니다. 위치 권한을 확인해 주세요.',
        );
      }
    } catch (error) {
  ...
  const handleOpenDirections = async () => {
    try {
      ...
      if (hasCoords) {
        if (Platform.OS === 'ios') {
          url = `http://maps.apple.com/?daddr=${lat},${lng}&dirflg=d&q=${encodeURIComponent(
            label,
          )}`;
        } else {
          url = `google.navigation:q=${lat},${lng}&mode=d`;
        ...
        <View style={styles.mapContainer}>
            ...
            <WebView
              ...
              onLoadEnd={() => {
                ...
                if (!initialBoundsRequestedRef.current && mapCenter) {
                  ...
                  const initialBounds = calculateBounds(
                    mapCenter.latitude,
                    mapCenter.longitude,
                    15,
                  );
              ...
              onMessage={event => {
                try {
                  ...
                  if (data.type === 'dragStart') {
                    ...
                    console.log(
                      '📍 드래그 종료 - API 호출 재개, 줌 레벨:',
                      data.zoomLevel || '알 수 없음',
                    );
                    ...
                    const pid =
                      typeof data.placeId === 'number'
                        ? data.placeId
                        : typeof data.id === 'string' &&
                          data.id.startsWith('poi-')
                        ? Number(data.id.replace('poi-', ''))
                        : NaN;
                    if (!isNaN(pid)) {
          ...
          <Animated.View
            style={[
              ...
              {
                bottom: isBottomSheetEnabled
                  ? Animated.add(bottomSheetHeight, 20)
                  : 20,
              },
            ]}>
            <TouchableOpacity
                ...
                <View style={styles.locationInfo}>
                  <View style={styles.locationHeader}>
                    <View style={styles.nameAndBadge}>
                      ...
                      <CongestionBadge
                        level={selectedLocation.congestionLevelNum || 0}
                        style={{marginRight: 8}}
                      />
                    </View>
                    <TouchableOpacity
                  ...
                  <View style={styles.chartSection}>
                    <View>
                      ...
                      {/* 요일별 평균 혼잡도 */}
                      {weekCongestionData &&
                        weekCongestionData.congestionsByDay.length > 0 && (
                          <View style={[styles.weekSummary, styles.grayCard]}>
                            ...
                            <View
                              ...
                              <View
                                style={[
                                  ...
                                  {
                                    backgroundColor: weekCongestionData
                                      ? getCongestionColor(
                                          weekCongestionData.realtimeLevel,
                                        )
                                      : '#ff4444',
                                  },
                                ]}
                              ...
                              <Text
                                style={[
                                  styles.congestionStatusText,
                                  {
                                    color: weekCongestionData
                                      ? getCongestionColor(
                                          weekCongestionData.realtimeLevel,
                                        )
                                      : '#ff4444',
                            ...
                            <View style={styles.weekBars}>
                              {weekCongestionData.congestionsByDay.map(
                                (val: number, index: number) => {
                                  const max = Math.max(
                                    ...weekCongestionData.congestionsByDay,
                                    1,
                                  );
                                  const height = Math.max(
                                    8,
                                    Math.round((val / max) * 60),
                                  );
                                  const isSelected = selectedDay === index;
                                  const isToday =
                                    index === weekCongestionData.standardDay;
                      ...
                      {/* 시간별 혼잡도 차트 */}
                      {weekCongestionData &&
                        weekCongestionData.congestionsByTime.length > 0 && (
                          <View style={[styles.timeSection, styles.grayCard]}>
                            ...
                            <View style={styles.chartWrapper}>
                              {(() => {
                                const dayDataRaw =
                                  weekCongestionData.congestionsByTime[
                                    selectedDay
                                  ] || [];
                                const timeDataRaw = dayDataRaw
                                  .slice(6, 24)
                                  .map((v: any) => Number(v) || 0);
                                // 값(1~5미만)을 1000배 하여 '명' 단위의 정수로 환산
                                const timeDataCounts = timeDataRaw.map(v =>
                                  Math.round(v * 1000),
                                );
                                    ...
                                    <View
                                      ...
                                      {timeDataCounts.map(
                                        (count: number, idx: number) => {
                                          ...
                                          const isCurrentTime =
                                            selectedDay ===
                                              weekCongestionData.standardDay &&
                                            actualHour ===
                                              weekCongestionData.standardTime;
                  ...
                  <View style={styles.infoBox}>
                    ...
                    <Text style={styles.infoText}>
                      {(() => {
                        if (
                          !selectedLocation?.name ||
                          !weekCongestionData?.congestionsByDay?.length
                        ) {
                          return '오후 7시 이후에는 비교적 여유로울 전망입니다.';
                        }

                        // 가장 혼잡도가 낮은 요일 찾기
                        const congestionsByDay =
                          weekCongestionData.congestionsByDay;
                        const minCongestionValue = Math.min(
                          ...congestionsByDay,
                        );
                        const minDayIndex = congestionsByDay.findIndex(
                          val => val === minCongestionValue,
                        );
                        const dayNames = [
                          '월요일',
                          '화요일',
                        ...
                        const isVowel =
                          /^[aeiouAEIOU가-기나-니다-디라-리마-미바-비사-시아-이자-지차-치카-키타-티파-피하-히]/.test(
                            placeName,
                          );
                    ...
                    <View style={styles.visitorSection}>
                      ...
                      {(() => {
                        ...
                        const ticks = Array.from(
                          {length: Math.floor(topTick / 10) + 1},
                          (_, i) => i * 10,
                        ).reverse();
                        return (
                  ...
                  <View style={styles.buttonContainer}>
                    <TouchableOpacity
                      ...
                      onPress={openDetailInAttraction}>
